...
이 회로의 main gain path는 BJT 3개를 중심으로 목표 40 dB gain에 접근하는 구조이다. 현재 BJT-only reference 후보는 `RC=120k`, `RE=18.5k`, `RB_TOP=3.3Meg`, `RB_BOT=1Meg`, `CIN=68n`, `C12=68n`, `C23=68n`, `CH1=CH2=CH3=30p`, `COUT=10n`, `ROUT_TOP=10Meg`, `ROUT_BOT=10Meg`인 `bjt3_sweep_coutalign_c10n`이다. 80 dB/dec high-frequency hard target을 위한 현재 performance fallback은 이 BJT gain path 뒤에 ideal ahdLib-equivalent OPAMP follower와 `RBUF1=RBUF2=10k`, `CBUF1=CBUF2=150p` 2-pole output filter를 추가한 `bjt3_sweep_hfbuf_r10k_c150p`이다.
...
High-frequency rolloff hard-target 검증을 위해 load-buffer/filter fallback도 별도로 평가했다. `bjt3_sweep_hfbuf_r10k_c150p` 후보는 BJT 3-stage gain path와 `COUT=10n`, `ROUT_TOP=ROUT_BOT=10Meg` rebias를 유지하고, rebias node 뒤에 ideal ahdLib-equivalent OPAMP follower와 2단 RC output filter(`RBUF1=RBUF2=10k`, `CBUF1=CBUF2=150p`)를 추가한다. OPAMP는 main gain path가 아니라 load buffer/filter fallback으로만 사용하며, PPA 전류 모델은 `OPAMP_FT=2Meg`, `IOPAMP=OPAMP_FT*7e-12`이다. 이 후보는 ngspice OP/AC/transient/noise와 공통 로그 검증을 통과했고, Cycle G deliverables도 이 fallback 기준으로 갱신했다. 최종 표는 `midgain=39.5403745 dB`, lower cutoff `10.598446 Hz`, upper cutoff `20738.048859 Hz`, transient `out_pp=0.189751 V`, output center `2.499753 V`, `worst_power=235.862 uW`, `area_p=265169812.927`, `ac_nrmse=0.105900`, `tran_nrmse=0.029755`이다. High-frequency slopes는 `10k-100k=-28.40 dB/dec`, `20k-200k=-48.90 dB/dec`, `100k-1Meg=-87.17 dB/dec`로 far-slope hard-target proxy를 통과했다. BJT-only `bjt3_sweep_coutalign_c10n` 대비 power는 약 `+70.0 uW`, area는 약 `+369337.75 p` 증가한다. 따라서 80 dB/dec high-frequency proxy가 hard target이면 `bjt3_sweep_hfbuf_r10k_c150p`를 제출 후보로 사용하고, ideal OPAMP fallback과 power penalty를 피하는 것이 우선이면 `bjt3_sweep_coutalign_c10n`을 low-power reference로 유지한다.

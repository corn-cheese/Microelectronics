...
const candidates = [
  { suffix: "10p", value: "10p", numeric: 10e-12 },
  { suffix: "15p", value: "15p", numeric: 15e-12 },
  { suffix: "22p", value: "22p", numeric: 22e-12 },
  { suffix: "33p", value: "33p", numeric: 33e-12 },
  { suffix: "47p", value: "47p", numeric: 47e-12 },
];
...
function baseNetlist(kind, candidate) {
  const version = `bjt3_sweep_hf4pole_chout${candidate.suffix}`;
  const source =
    kind === "tran"
...
write ${rawDir}/${version}_ac.raw frequency v(vin) v(b3_collector) v(vout_final) gain gain_vv gain_db phase_deg`,
...
wrdata ${csvDir}/${version}_tran.csv v(vin) v(b3_collector) v(vout_final) i(vdd)
...
wrdata ${csvDir}/${version}_noise_10hz_20khz_total.csv onoise_total inoise_total`,
  ...
  return `* SKY130 BJT 3-stage high-frequency fourth-pole ${kind}: CHOUT=${candidate.value}
...
async function summarizeCandidate(candidate) {
  ...
  const pdc = 5 * istatic;
  return {
    version: `hf4pole_chout${candidate.suffix}`,
    stem: version,
    candidate,
    changed_family: "high_cutoff_shape",
    changed_values: `CHOUT=${candidate.value} added from vout_final to ground; CH1=CH2=CH3=30p; CLOAD_10P=10p`,
...
async function addNoiseTotals(row) {
  const logs = [await fs.readFile(`${logDir}/${row.stem}_noise.log`, "utf8")];
  const scan = logs.map(logScanText).filter(Boolean).join(" | ");
  const log = logs[0];
...
const validRows = rows.filter((row) =>
  ...
  row.high_slope_100k_1meg <= -70
);

const selected = validRows.sort((a, b) => a.high_slope_100k_1meg - b.high_slope_100k_1meg)[0] ?? null;
if (selected) {
  selected.decision = "accepted";
  selected.notes = "log scan pass; selected because it adds a deliberate fourth high-frequency pole, keeps the -3 dB upper cutoff above 20 kHz, preserves 10 pF transient behavior, and improves far high-frequency rolloff toward 80 dB/dec";
...
for (const row of rows) {
  ...
  if (row.log_scan) {
    ...
    row.notes = "log scan pass; rejected because the added fourth-pole capacitance moves the upper cutoff below the 20 kHz bandwidth target";
  } else if (row.high_slope_100k_1meg > -70) {
    row.decision = "rejected_gain";
    row.notes = "log scan pass; valid gain/bandwidth/transient behavior, but far high-frequency rolloff is still weaker than the 4-pole target";
  } else {
    row.decision = "rejected_power";
    row.notes = "log scan pass; rejected in favor of the selected candidate with stronger high-frequency rolloff at comparable DC power";
  }
}
...
const tableRows = rows.map((row) => ({
  ...
  pdc_w: fmt(row.pdc_w),
  estimated_area_note: `adds one sky130 cap_vpp-equivalent intentional output pole capacitor of ${row.candidate.value}; exact area deferred to next Cycle G refresh if accepted`,
  decision: row.decision,
  notes: `${row.notes}; high slopes 10k-100k=${fmt(row.high_slope_10k_100k, 2)} dB/dec, 20k-200k=${fmt(row.high_slope_20k_200k, 2)} dB/dec, 100k-1Meg=${fmt(row.high_slope_100k_1meg, 2)} dB/dec${Number.isFinite(row.inoise_total) ? `; target-band noise inoise=${fmt(row.inoise_total)} Vrms` : ""}`,
}));
...
await fs.writeFile(`${tableDir}/bjt3_sweep_hf4pole_summary.csv`, toCsv(headers, tableRows));
...
if (selected) {
  await fs.writeFile(`${tableDir}/${selected.stem}_summary.csv`, toCsv(["metric", "value", "unit", "notes"], [
    ...
    { metric: "decision", value: "accepted", unit: "", notes: "Accepted fourth-pole candidate; Cycle G deliverables need refresh" },
  ]));
}

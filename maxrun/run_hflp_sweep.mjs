...
const candidates = [
  { rSuffix: "47k", rlp: "47k", rlpNumeric: 47e3, cSuffix: "47p", clp: "47p", clpNumeric: 47e-12 },
  { rSuffix: "68k", rlp: "68k", rlpNumeric: 68e3, cSuffix: "33p", clp: "33p", clpNumeric: 33e-12 },
  { rSuffix: "68k", rlp: "68k", rlpNumeric: 68e3, cSuffix: "47p", clp: "47p", clpNumeric: 47e-12 },
  { rSuffix: "100k", rlp: "100k", rlpNumeric: 100e3, cSuffix: "22p", clp: "22p", clpNumeric: 22e-12 },
  { rSuffix: "100k", rlp: "100k", rlpNumeric: 100e3, cSuffix: "33p", clp: "33p", clpNumeric: 33e-12 },
  { rSuffix: "150k", rlp: "150k", rlpNumeric: 150e3, cSuffix: "15p", clp: "15p", clpNumeric: 15e-12 },
  { rSuffix: "150k", rlp: "150k", rlpNumeric: 150e3, cSuffix: "22p", clp: "22p", clpNumeric: 22e-12 },
];
...
function stem(candidate) {
  return `bjt3_sweep_hflp_r${candidate.rSuffix}_c${candidate.cSuffix}`;
}
...
RLP vout_drv vout_final ${candidate.rlp}
CLP vout_final 0 ${candidate.clp}
...
wrdata ${csvDir}/${version}_noise_10hz_20khz_total.csv onoise_total inoise_total`,
  ...
  return `* SKY130 BJT 3-stage explicit output low-pass section ${kind}: RLP=${candidate.rlp}, CLP=${candidate.clp}, CLOAD=10p
...
async function summarizeCandidate(candidate) {
  ...
  const pdc = 5 * istatic;
  const totalCap = candidate.clpNumeric + 10e-12;
  return {
    version: `hflp_r${candidate.rSuffix}_c${candidate.cSuffix}`,
    stem: version,
    candidate,
    changed_family: "high_cutoff_shape",
    changed_values: `RLP=${candidate.rlp} between vout_drv and vout_final; CLP=${candidate.clp} at vout_final; CLOAD_10P=10p retained at vout_final; CH1=CH2=CH3=30p`,
    ...
    istatic_a: istatic,
    pdc_w: pdc,
    output_lp_pole_hz: 1 / (2 * Math.PI * candidate.rlpNumeric * totalCap),
    log_scan: scan,
    decision: "",
    notes: "",
  };
...
if (selected) {
  ...
  selected.notes = "log scan pass; selected because the explicit output RC low-pass section adds a stronger independent high-frequency pole while keeping the -3 dB upper cutoff at or above 20 kHz and preserving 10 pF transient behavior";
...
for (const row of rows) {
  ...
  if (row.log_scan) {
    ...
    row.notes = "log scan pass; rejected because the explicit output low-pass pole moves the upper cutoff below the 20 kHz bandwidth target";
    ...
    row.notes = "log scan pass; rejected in favor of the candidate with an upper cutoff closer to 20 kHz";
  }
}
...
const tableRows = rows.map((row) => ({
  ...
  pdc_w: fmt(row.pdc_w),
  estimated_area_note: `adds one output low-pass resistor RLP=${row.candidate.rlp} and one internal capacitor CLP=${row.candidate.clp}; external CLOAD_10P remains ppa_included=false`,
  decision: row.decision,
  notes: `${row.notes}; output LP pole with CLP+10p=${fmt(row.output_lp_pole_hz, 2)} Hz; high slopes 10k-100k=${fmt(row.high_slope_10k_100k, 2)} dB/dec, 20k-200k=${fmt(row.high_slope_20k_200k, 2)} dB/dec, 100k-1Meg=${fmt(row.high_slope_100k_1meg, 2)} dB/dec${Number.isFinite(row.inoise_total) ? `; target-band noise inoise=${fmt(row.inoise_total)} Vrms` : ""}`,
}));

await fs.writeFile(`${tableDir}/bjt3_sweep_hflp_summary.csv`, toCsv(headers, tableRows));
...
if (selected) {
  await fs.writeFile(`${tableDir}/${selected.stem}_summary.csv`, toCsv(["metric", "value", "unit", "notes"], [
    ...
    { metric: "output_lp_pole_hz", value: fmt(selected.output_lp_pole_hz, 2), unit: "Hz", notes: "1/(2*pi*RLP*(CLP+10pF))" },
    ...
    { metric: "decision", value: "accepted", unit: "", notes: "Accepted explicit output low-pass candidate; Cycle G deliverables need refresh" },
  ]));
}

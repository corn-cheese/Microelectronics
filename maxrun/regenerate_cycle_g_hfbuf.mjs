...
const version = "bjt3_sweep_hfbuf_r10k_c150p";
...
const priorPerformanceRows = await fs.readFile(`${tablesDir}/performance_summary.csv`, "utf8")
  .then(parseCsv)
  .catch(() => []);
const bjtOnlyPerformance = priorPerformanceRows.find((row) => row.version === "bjt3_sweep_coutalign_c10n");
...
const areaRows = [
  ...
  {
    component: "EBUF/IOPAMP",
    logical_role: "ideal ahdLib-equivalent output load buffer",
    library_cell: "ahdLib / opamp",
    value: "OPAMP_FT=2Meg; IOPAMP=14uA",
    geometry: "area=1000p by assignment rule; current=ft*7e-12",
    area_formula: "1000p * count",
    area_p: "1000",
    ppa_included: "true",
    notes: "fallback buffer isolates the BJT gain path from the two-pole output filter; ideal buffer noise is not modeled",
  },
  {
    component: "RBUF1-RBUF2",
    logical_role: "post-buffer two-pole output filter resistors",
    library_cell: resistorCell,
    value: "10000",
    geometry: `w=5.73um; l=${fmt(resLength(10000))}um; count=2`,
    area_formula: "((R-rcon)/rsheet) * w * count, with rcon=68.05 ohm and rsheet=56.46 ohm/um from PDK model",
    area_p: fmt(resArea(10000, 2)),
    ppa_included: "true",
    notes: "adds two explicit high-frequency filter poles after the output buffer",
  },
  {
    component: "CBUF1-CBUF2",
    logical_role: "post-buffer two-pole output filter capacitors",
    library_cell: capCell,
    value: "1.5E-10",
    geometry: `11.5um x 11.7um; mult=${fmt(capMult(150e-12))}; count=2`,
    area_formula: "11.5 * 11.7 * (C / 110.19fF) * count, using PDK model ctot_a",
    area_p: fmt(capArea(150e-12, 2)),
    ppa_included: "true",
    notes: "two internal 150 pF filter capacitors; external 10 pF load remains excluded from PPA",
  },
  {
    component: "CLOAD_10P",
...
const deviceRows = [
  ...
  ["EBUF/IOPAMP", "ideal ahdLib-equivalent output load buffer", "ahdLib / opamp", "OPAMP_FT=2Meg, IOPAMP=14uA", "area=1000p by assignment rule", "1", "fallback output buffer only; not part of the main BJT voltage gain path"],
  ["RBUF1-RBUF2", "post-buffer two-pole output filter resistors", resistorCell, "10k", "w=5.73um, length calculated from model", "2", "explicit output filter section for high-frequency rolloff"],
  ["CBUF1-CBUF2", "post-buffer two-pole output filter capacitors", capCell, "150p", "11.5um x 11.7um, multiplier calculated from model ctot_a", "2", "explicit output filter section for high-frequency rolloff"],
  ...
  geometry,
  count,
  source_netlist: "netlists/bjt3_sweep_hfbuf_r10k_c150p_ac.spice",
...
await fs.writeFile(`${tablesDir}/performance_summary.csv`, toCsv(["version", "load_pf", "midband_gain_db", "lower_cutoff_hz", "upper_cutoff_hz", "low_rolloff_db_dec", "high_rolloff_db_dec", "passband_ripple_db", "ac_nrmse", "tran_nrmse", "power_w", "area_p", "decision", "notes"], [{
  ...
  notes: `Load-buffer/filter fallback candidate; keeps the 3-stage BJT gain path and adds one ideal ahdLib-equivalent OPAMP follower plus two 10k/150p output RC filter poles; 10 pF loaded; ideal buffer noise is not modeled. AC nRMSE uses gain dB over 1 Hz to 1 MHz; transient nRMSE uses 2.5 V - 100*(vin-2.5) target from 50 ms to 100 ms; low slopes 0.316-3.162Hz=${fmt(lowSlopeFar, 2)} dB/dec and 1-10Hz=${fmt(lowSlopeNear, 2)} dB/dec; high slopes 10k-100k=${fmt(highSlope10k100k, 2)} dB/dec, 20k-200k=${fmt(highSlope20k200k, 2)} dB/dec, 100k-1Meg=${fmt(highSlope100k1m, 2)} dB/dec`,
}]));

if (bjtOnlyPerformance) {
  const num = (value) => Number(value);
  const farSlopeMatch = bjtOnlyPerformance.notes?.match(/100k-1Meg=([-+0-9.]+) dB\/dec/);
  const bjtFarSlope = farSlopeMatch ? Number(farSlopeMatch[1]) : "";
  const comparisonHeaders = [
    ...
    "power_w",
    "area_p",
    "power_delta_vs_bjt_only_w",
    "area_delta_vs_bjt_only_p",
    "decision_note",
  ];
  await fs.writeFile(`${tablesDir}/bjt3_final_candidate_comparison.csv`, toCsv(comparisonHeaders, [
    {
      version: bjtOnlyPerformance.version,
      role: "bjt_only_primary",
      midband_gain_db: bjtOnlyPerformance.midband_gain_db,
      lower_cutoff_hz: bjtOnlyPerformance.lower_cutoff_hz,
      upper_cutoff_hz: bjtOnlyPerformance.upper_cutoff_hz,
      high_rolloff_100k_1meg_db_dec: bjtFarSlope,
      ac_nrmse: bjtOnlyPerformance.ac_nrmse,
      tran_nrmse: bjtOnlyPerformance.tran_nrmse,
      power_w: bjtOnlyPerformance.power_w,
      area_p: bjtOnlyPerformance.area_p,
      power_delta_vs_bjt_only_w: "0",
      area_delta_vs_bjt_only_p: "0",
      decision_note: "Primary low-power BJT-only submission candidate; misses the far high-frequency 80 dB/dec proxy.",
    },
    {
      version,
      role: "load_buffer_filter_fallback",
      midband_gain_db: fmt(midDb),
      lower_cutoff_hz: fmt(lower),
      upper_cutoff_hz: fmt(upper),
      high_rolloff_100k_1meg_db_dec: fmt(highSlope100k1m),
      ac_nrmse: fmt(acNrmse),
      tran_nrmse: fmt(tranNrmse),
      power_w: fmt(worstPower),
      area_p: fmt(areaTotal),
      power_delta_vs_bjt_only_w: fmt(worstPower - num(bjtOnlyPerformance.power_w)),
      area_delta_vs_bjt_only_p: fmt(areaTotal - num(bjtOnlyPerformance.area_p)),
      decision_note: "Performance fallback for a hard high-frequency rolloff target; adds OPAMP/filter area and about 70 uW static power.",
    },
  ]));
}

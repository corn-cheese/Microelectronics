...
const candidates = [
  ...
  { suffix: "100k", value: "100k", numeric: 100e3 },
  { suffix: "150k", value: "150k", numeric: 150e3 },
  { suffix: "220k", value: "220k", numeric: 220e3 },
...
RISO vout_drv vout_final ${candidate.value}
...
wrdata ${csvDir}/${version}_noise_10hz_20khz_total.csv onoise_total inoise_total`,
  ...
  return `* SKY130 BJT 3-stage high-frequency output-isolation ${kind}: RISO=${candidate.value}, CLOAD=10p
...
async function summarizeCandidate(candidate) {
  ...
  const pdc = 5 * istatic;
  return {
    version: `hfiso_r${candidate.suffix}`,
    stem: version,
    candidate,
    changed_family: "high_cutoff_shape",
    changed_values: `RISO=${candidate.value} inserted between vout_drv and vout_final; CLOAD_10P=10p at vout_final; CH1=CH2=CH3=30p`,
    ...
    istatic_a: istatic,
    pdc_w: pdc,
    riso_pole_hz: 1 / (2 * Math.PI * candidate.numeric * 10e-12),
    log_scan: scan,
    decision: "",
    notes: "",
  };
...
for (const row of rows) {
  ...
  if (row.log_scan) {
    ...
    row.notes = "log scan pass; rejected because the isolation/load pole moves the upper cutoff below the 20 kHz bandwidth target";
  } else if (row.high_slope_100k_1meg > -75) {
    row.decision = "rejected_gain";
    row.notes = "log scan pass; valid gain/bandwidth/transient behavior, but far high-frequency rolloff is still weaker than the selected 4-pole target threshold";
  } else {
    row.decision = "rejected_power";
    row.notes = "log scan pass; rejected in favor of the selected candidate with an upper cutoff closer to the 20 kHz target";
  }
}
...
const tableRows = rows.map((row) => ({
  ...
  pdc_w: fmt(row.pdc_w),
  estimated_area_note: `adds one series output-isolation resistor RISO=${row.candidate.value}; CLOAD_10P remains external and ppa_included=false; exact resistor area deferred to next Cycle G refresh if accepted`,
...
await fs.writeFile(`${tableDir}/bjt3_sweep_hfiso_summary.csv`, toCsv(headers, tableRows));
...
if (selected) {
  await fs.writeFile(`${tableDir}/${selected.stem}_summary.csv`, toCsv(["metric", "value", "unit", "notes"], [
    ...
    { metric: "decision", value: "accepted", unit: "", notes: "Accepted output-isolated fourth-pole candidate; Cycle G deliverables need refresh" },
  ]));
}

...
const runId = "run_009";
const family = "low_frequency_rolloff_pressure";
const hypothesis = "Push the existing input, interstage, and output coupling poles upward around the run-8 operating point to test whether stronger low-frequency rolloff can improve assignment alignment without collapsing overall relaxed PPA.";
const accepted2Bjt = {
  version: "bjt2_sweep_ppa_rc120k_re3p9k_rb4p7m_c68n_cout10n_ch22p_r10k_c220p_ft250k",
  midgain_db: 39.7601021,
  lower_cutoff_hz: 12.56994968,
  upper_cutoff_hz: 20409.38025254,
  high_slope_100k_1meg_db_dec: -80.5594,
  ac_nrmse: 0.136319434501,
  tran_nrmse: 0.023733677809,
  power_w: 6.517243343856e-5,
  area_p: 1.820824e8,
};
const fallback3BjtVersion = "bjt3_sweep_hfbuf_r10k_c150p";
const LF_SLOPE_PRESSURE_TARGET = 60;

function valueToken(value, unit) {
  const scaled = Number((value / unit.scale).toPrecision(12));
  return `${String(scaled).replace(".", "p")}${unit.name}`;
}

function spiceValueToken(value, unit) {
  return `${Number((value / unit.scale).toPrecision(12))}${unit.name}`;
}

function buildCandidates() {
  const cinValues = [33e-9, 47e-9, 56e-9, 68e-9];
  const c12Values = [33e-9, 47e-9, 56e-9, 68e-9];
  const coutValues = [4.7e-9, 6.8e-9, 10e-9];
  const chValues = [27e-12, 28e-12];
  const rbufValues = [11e3];
  const candidates = [];
  for (const cinNumeric of cinValues) {
    for (const c12Numeric of c12Values) {
      for (const coutNumeric of coutValues) {
        for (const chNumeric of chValues) {
          for (const rbufNumeric of rbufValues) {
          const cinToken = valueToken(cinNumeric, { scale: 1e-9, name: "n" });
          const c12Token = valueToken(c12Numeric, { scale: 1e-9, name: "n" });
          const coutToken = valueToken(coutNumeric, { scale: 1e-9, name: "n" });
          const chToken = valueToken(chNumeric, { scale: 1e-12, name: "p" });
          const rbufToken = valueToken(rbufNumeric, { scale: 1e3, name: "k" });
          const suffix = `lfrp_rc125k_re4p05k_rb4p7m_cin${cinToken}_c12_${c12Token}_cout${coutToken}_ch${chToken}_r${rbufToken}_c180p_ft175k`;
          candidates.push({
            suffix,
            family,
            hypothesis,
            complexityNote: "no new passive topology; pressures the three existing high-pass coupling poles around the run-8 operating point",
            rc: "125k",
            rcNumeric: 125e3,
            re: "4.05k",
            reNumeric: 4.05e3,
            ...
            rbBot: "1Meg",
            rbBotNumeric: 1e6,
            cin: spiceValueToken(cinNumeric, { scale: 1e-9, name: "n" }),
            cinNumeric,
            c12: spiceValueToken(c12Numeric, { scale: 1e-9, name: "n" }),
            c12Numeric,
            cout: spiceValueToken(coutNumeric, { scale: 1e-9, name: "n" }),
            coutNumeric,
            rout: "10Meg",
            routNumeric: 10e6,
            ch: spiceValueToken(chNumeric, { scale: 1e-12, name: "p" }),
            chNumeric,
            rbuf: spiceValueToken(rbufNumeric, { scale: 1e3, name: "k" }),
            rbufNumeric,
            cbuf: "180p",
            cbufNumeric: 180e-12,
            opampFt: "175k",
            opampFtNumeric: 175e3,
          });
          }
        }
      }
  ...
  return candidates.filter((candidate) =>
    candidate.suffix !== "lfrp_rc125k_re4p05k_rb4p7m_cin68n_c12_68n_cout10n_ch27p_r11k_c180p_ft175k"
  );
}
...
function stem(candidate) {
  return `bjt2_autotopo_${candidate.suffix}`;
}
...
C12 b1_collector b2_base {C12VAL}
...
.param RE1VAL=${candidate.re}
.param RE2VAL=${candidate.re}
.param CINVAL=${candidate.cin}
.param C12VAL=${candidate.c12}
.param COUT=${candidate.cout}
.param CH=${candidate.ch}
.param ROUT=${candidate.rout}
...
function estimateArea(candidate) {
  const outputCapArea = Number.isFinite(candidate.cout1Numeric) && Number.isFinite(candidate.cout2Numeric)
    ? capArea(candidate.cout1Numeric, 1) + capArea(candidate.cout2Numeric, 1)
    : capArea(candidate.coutNumeric, 1);
  const couplingCapArea = Number.isFinite(candidate.cinNumeric) && Number.isFinite(candidate.c12Numeric)
    ? capArea(candidate.cinNumeric, 1) + capArea(candidate.c12Numeric, 1)
    : capArea(candidate.ccplNumeric, 2);
  const outputMidpointRebiasArea = Number.isFinite(candidate.rhpNumeric) ? resArea(candidate.rhpNumeric, 2) : 0;
  const outputFinalRebiasArea = resArea(candidate.routNumeric ?? 10e6, 2);
  const emitterResistorArea = Number.isFinite(candidate.re2TopNumeric) && Number.isFinite(candidate.re2BotNumeric)
    ? resArea(candidate.re1Numeric ?? candidate.reNumeric, 1) + resArea(candidate.re2TopNumeric, 1) + resArea(candidate.re2BotNumeric, 1)
    : resArea(candidate.reNumeric, 2);
  const emitterBypassArea = Number.isFinite(candidate.cebyp2Numeric) ? capArea(candidate.cebyp2Numeric, 1) : 0;
  return [
    2,
    ...
    outputMidpointRebiasArea,
    outputFinalRebiasArea,
    emitterBypassArea,
    couplingCapArea,
    outputCapArea,
...
async function summarizeCandidate(candidate) {
  ...
  return {
    ...
    gain_at_10hz_relative_to_midband_db: interpLog(acRows, 10, "gain_db") - midgainDb,
    gain_at_20khz_relative_to_midband_db: interpLog(acRows, 20000, "gain_db") - midgainDb,
...
function candidateScore(row) {
  const weighted_ac_nrmse = row.ac_nrmse / accepted2Bjt.ac_nrmse;
  const weighted_tran_nrmse = 0.5 * row.tran_nrmse / accepted2Bjt.tran_nrmse;
  const area_ratio_vs_current = row.estimated_area_p / accepted2Bjt.area_p;
  const power_ratio_vs_current = row.worst_power_w / accepted2Bjt.power_w;
  const bandwidth_penalty =
    row.upper_cutoff_hz >= 20000 ? 0 :
    row.upper_cutoff_hz >= 15000 ? (20000 - row.upper_cutoff_hz) / 5000 :
    10 + (15000 - row.upper_cutoff_hz) / 1000;
  const rolloff_penalty =
    row.high_slope_100k_1meg <= -80 ? 0 :
    row.high_slope_100k_1meg <= -70 ? (row.high_slope_100k_1meg + 80) / 5 :
    10 + (row.high_slope_100k_1meg + 70) / 2;
  const low_frequency_shape_penalty =
    Math.abs(row.gain_at_10hz_relative_to_midband_db + 3) / 3 +
    Math.max(0, 40 - row.low_slope_1_10) / 40;
  const lf_rolloff_pressure_penalty =
    row.candidate.family === "low_frequency_rolloff_pressure"
      ? 0.6 * Math.max(0, LF_SLOPE_PRESSURE_TARGET - row.low_slope_1_10) / LF_SLOPE_PRESSURE_TARGET
      : 0;
  const complexity_penalty = row.candidate.family === "hf_margin_power_retune" ||
    row.candidate.family === "hf_margin_robustness_retune" ||
    row.candidate.family === "collector_emitter_operating_point_rebalance" ||
    row.candidate.family === "coupling_pole_skew_rebalance" ||
    row.candidate.family === "low_frequency_rolloff_pressure" ? 0.02 :
    row.candidate.family === "limited_emitter_bypass_shaping" ? 0.09 :
    row.candidate.family === "area_aware_output_split" ? 0.14 : 0.05;
  row.score_components = {
    weighted_ac_nrmse,
    weighted_tran_nrmse,
    area_ratio_vs_current,
    power_ratio_vs_current,
    bandwidth_penalty,
    rolloff_penalty,
    low_frequency_shape_penalty,
    lf_rolloff_pressure_penalty,
    complexity_penalty,
  };
  return Object.values(row.score_components).reduce((sum, value) => sum + value, 0);
}

function makeAreaRows(row) {
  const c = row.candidate;
  const emitterRows = Number.isFinite(c.re2TopNumeric) && Number.isFinite(c.re2BotNumeric)
    ? [
      ["RE1", "first-stage emitter degeneration resistor", resistorCell, c.re1Numeric ?? c.reNumeric, `w=5.73um; l=${fmt(resLength(c.re1Numeric ?? c.reNumeric))}um; count=1`, "((R-rcon)/rsheet) * w * count", resArea(c.re1Numeric ?? c.reNumeric, 1), true, "first-stage gain/headroom setting"],
      ["RE2_TOP", "unbypassed second-stage emitter degeneration segment", resistorCell, c.re2TopNumeric, `w=5.73um; l=${fmt(resLength(c.re2TopNumeric))}um; count=1`, "((R-rcon)/rsheet) * w * count", resArea(c.re2TopNumeric, 1), true, "sets high-frequency residual second-stage emitter degeneration"],
      ["RE2_BOT", "bypassed second-stage emitter degeneration segment", resistorCell, c.re2BotNumeric, `w=5.73um; l=${fmt(resLength(c.re2BotNumeric))}um; count=1`, "((R-rcon)/rsheet) * w * count", resArea(c.re2BotNumeric, 1), true, "retains accepted DC emitter resistance"],
      ["CEBYP2", "second-stage emitter bypass shaping capacitor", capCell, c.cebyp2Numeric, `11.5um x 11.7um; mult=${fmt(capMult(c.cebyp2Numeric))}; count=1`, "11.5 * 11.7 * (C / 110.19fF) * count", capArea(c.cebyp2Numeric, 1), true, "passive emitter degeneration bypass shaping"],
    ]
    : [
  ...
  const outputRows = Number.isFinite(c.cout1Numeric) && Number.isFinite(c.cout2Numeric)
    ? [
      ["COUT1", "first half of split output high-pass network", capCell, c.cout1Numeric, `11.5um x 11.7um; mult=${fmt(capMult(c.cout1Numeric))}; count=1`, "11.5 * 11.7 * (C / 110.19fF) * count", capArea(c.cout1Numeric, 1), true, "new output shaping capacitor from CE2 collector to midpoint"],
      ["RHP_TOP-RHP_BOT", "split output midpoint rebias divider", resistorCell, c.rhpNumeric, `w=5.73um; l=${fmt(resLength(c.rhpNumeric))}um; count=2`, "((R-rcon)/rsheet) * w * count", resArea(c.rhpNumeric, 2), true, "sets split output midpoint common-mode"],
      ["COUT2", "second half of split output high-pass network", capCell, c.cout2Numeric, `11.5um x 11.7um; mult=${fmt(capMult(c.cout2Numeric))}; count=1`, "11.5 * 11.7 * (C / 110.19fF) * count", capArea(c.cout2Numeric, 1), true, "new output shaping capacitor from midpoint to output rebias node"],
    ]
    : [
      ["COUT", "output coupling capacitor before rebias divider", capCell, c.coutNumeric, `11.5um x 11.7um; mult=${fmt(capMult(c.coutNumeric))}; count=1`, "11.5 * 11.7 * (C / 110.19fF) * count", capArea(c.coutNumeric, 1), true, "accepted output rebias coupling capacitor"],
    ];
  const couplingRows = Number.isFinite(c.cinNumeric) && Number.isFinite(c.c12Numeric)
    ? [
      ["CIN", "input coupling capacitor", capCell, c.cinNumeric, `11.5um x 11.7um; mult=${fmt(capMult(c.cinNumeric))}; count=1`, "11.5 * 11.7 * (C / 110.19fF) * count", capArea(c.cinNumeric, 1), true, "input high-pass pole placement"],
      ["C12", "interstage coupling capacitor", capCell, c.c12Numeric, `11.5um x 11.7um; mult=${fmt(capMult(c.c12Numeric))}; count=1`, "11.5 * 11.7 * (C / 110.19fF) * count", capArea(c.c12Numeric, 1), true, "interstage high-pass pole placement"],
    ]
    : [
  ...
  return [
    ...
    ...couplingRows,
    ["ROUT_TOP-ROUT_BOT", "output rebias divider", resistorCell, c.routNumeric ?? 10e6, `w=5.73um; l=${fmt(resLength(c.routNumeric ?? 10e6))}um; count=2`, "((R-rcon)/rsheet) * w * count", resArea(c.routNumeric ?? 10e6, 2), true, "sets final output common-mode to 2.5 V"],
    ...outputRows,
...
async function writeDeliverables(rows, selected) {
  ...
  await fs.writeFile(`${tableDir}/bjt2_device_list.csv`, toCsv(["version", "component", "logical_role", "library_cell", "value", "geometry", "count", "source_netlist", "ppa_included", "notes"], areaRows.map((row) => ({
    ...
    count: row.component.includes("Q1-Q2") || row.component.includes("RC1-RC2") || row.component.includes("RE1-RE2") || row.component.includes("RB") || row.component.includes("RHP") || row.component.includes("ROUT") || row.component.includes("CH1-CH2") || row.component.includes("RBUF") || row.component.includes("CBUF") || row.component.includes("CIN,C12") ? "2" : "1",
...
async function readPriorAutotopoBest() {
  const indexPath = `${tableDir}/bjt2_autotopo_run_index.csv`;
  const rows = await fs.readFile(indexPath, "utf8").then(parseCsv).catch(() => []);
  const complete = rows
    .filter((row) => row.run_id !== runId)
    .filter((row) => row.best_version && Number.isFinite(Number(row.score)))
    .sort((a, b) => Number(a.score) - Number(b.score));
  return complete[0] ?? null;
}

async function appendRunIndex(selected, priorBest) {
  const indexPath = `${tableDir}/bjt2_autotopo_run_index.csv`;
  const headers = [
    "run_id",
    "timestamp",
    ...
    "upper_cutoff_hz",
    "high_slope_100k_1meg_db_dec",
    "delta_score_vs_prior_best",
    "decision",
    "next_action",
  ];
  const existing = await fs.readFile(indexPath, "utf8").catch(() => "");
  const deltaScore = priorBest ? selected.score - Number(priorBest.score) : "";
  const row = {
    run_id: runId,
    timestamp: new Date().toISOString(),
    branch: "codex/2stage-bjt-autotopo-ppa",
    family,
    hypothesis,
    best_version: selected.stem,
    score: fmt(selected.score, 8),
    ac_nrmse: fmt(selected.ac_nrmse, 12),
    tran_nrmse: fmt(selected.tran_nrmse, 12),
    power_w: fmt(selected.worst_power_w, 12),
    area_p: fmt(selected.estimated_area_p, 6),
    lower_cutoff_hz: fmt(selected.lower_cutoff_hz, 6),
    upper_cutoff_hz: fmt(selected.upper_cutoff_hz, 6),
    high_slope_100k_1meg_db_dec: fmt(selected.high_slope_100k_1meg, 4),
    delta_score_vs_prior_best: deltaScore === "" ? "" : fmt(deltaScore, 8),
    decision: selected.decision,
    next_action: selected.decision === "promote" ? "review for handoff candidate" : "keep run_008 as handoff unless LF pressure finds a total-PPA win",
  };
  const retainedRows = existing.trim()
    ? parseCsv(existing).filter((existingRow) => existingRow.run_id !== runId)
    : [];
  await fs.writeFile(indexPath, toCsv(headers, [...retainedRows, row]));
}

async function writeAutotopoDeliverables(rows, selected, priorBest) {
  const sweepHeaders = [
    "run_id",
    "family",
    "hypothesis",
  ...
  await fs.writeFile(`${tableDir}/bjt2_autotopo_ppa_sweep_summary.csv`, toCsv(sweepHeaders, rows.map((row) => ({
    run_id: runId,
    family: row.candidate.family,
    hypothesis: row.candidate.hypothesis,
    version: row.stem,
    midgain_db: fmt(row.midgain_db, 6),
    lower_cutoff_hz: fmt(row.lower_cutoff_hz, 6),
    upper_cutoff_hz: fmt(row.upper_cutoff_hz, 6),
    gain_at_10hz_relative_to_midband_db: fmt(row.gain_at_10hz_relative_to_midband_db, 6),
    gain_at_20khz_relative_to_midband_db: fmt(row.gain_at_20khz_relative_to_midband_db, 6),
    low_slope_1hz_10hz_db_dec: fmt(row.low_slope_1_10, 4),
    high_slope_100k_1meg_db_dec: fmt(row.high_slope_100k_1meg, 4),
    passband_ripple_db: fmt(row.passband_ripple_db, 6),
    ac_nrmse: fmt(row.ac_nrmse, 12),
    tran_nrmse: fmt(row.tran_nrmse, 12),
    ...
    worst_power_w: fmt(row.worst_power_w, 12),
    estimated_area_p: fmt(row.estimated_area_p, 6),
    complexity_note: row.candidate.complexityNote,
    weighted_ac_nrmse: fmt(row.score_components.weighted_ac_nrmse, 8),
    weighted_tran_nrmse: fmt(row.score_components.weighted_tran_nrmse, 8),
    area_ratio_vs_current: fmt(row.score_components.area_ratio_vs_current, 8),
    power_ratio_vs_current: fmt(row.score_components.power_ratio_vs_current, 8),
    bandwidth_penalty: fmt(row.score_components.bandwidth_penalty, 8),
    rolloff_penalty: fmt(row.score_components.rolloff_penalty, 8),
    low_frequency_shape_penalty: fmt(row.score_components.low_frequency_shape_penalty, 8),
    lf_rolloff_pressure_penalty: fmt(row.score_components.lf_rolloff_pressure_penalty, 8),
    complexity_penalty: fmt(row.score_components.complexity_penalty, 8),
    score: fmt(row.score, 8),
    decision: row.decision,
    notes: row.notes,
    log_scan: row.log_scan,
  }))));

  const bjt3Rows = await fs.readFile(`${tableDir}/bjt3_final_candidate_comparison.csv`, "utf8").then(parseCsv).catch(() => []);
  const fallback3 = bjt3Rows.find((row) => row.version === fallback3BjtVersion) ?? {};
  const comparisonHeaders = [
    "version",
    "role",
    "score",
    ...
    "power_w",
    "area_p",
    "power_delta_vs_selected_w",
    "area_delta_vs_selected_p",
    "ac_nrmse_delta_vs_selected",
    "tran_nrmse_delta_vs_selected",
    "notes",
  ];
  const baselineRows = [
    {
      version: accepted2Bjt.version,
      role: "accepted_2bjt_current",
      score: "",
      midgain_db: accepted2Bjt.midgain_db,
      lower_cutoff_hz: accepted2Bjt.lower_cutoff_hz,
      upper_cutoff_hz: accepted2Bjt.upper_cutoff_hz,
      high_slope_100k_1meg_db_dec: accepted2Bjt.high_slope_100k_1meg_db_dec,
      ac_nrmse: accepted2Bjt.ac_nrmse,
      tran_nrmse: accepted2Bjt.tran_nrmse,
      power_w: accepted2Bjt.power_w,
      area_p: accepted2Bjt.area_p,
      notes: "current accepted 2BJT baseline",
    },
    priorBest ? {
      version: priorBest.best_version,
      role: "prior_autotopo_best",
      score: priorBest.score,
      midgain_db: "",
      lower_cutoff_hz: priorBest.lower_cutoff_hz,
      upper_cutoff_hz: priorBest.upper_cutoff_hz,
      high_slope_100k_1meg_db_dec: priorBest.high_slope_100k_1meg_db_dec,
      ac_nrmse: priorBest.ac_nrmse,
      tran_nrmse: priorBest.tran_nrmse,
      power_w: priorBest.power_w,
      area_p: priorBest.area_p,
      notes: "best prior bjt2_autotopo_run_index row",
    } : null,
    {
      version: fallback3.version ?? fallback3BjtVersion,
      role: "3bjt_performance_fallback",
      score: "",
      midgain_db: fallback3.midband_gain_db,
      lower_cutoff_hz: fallback3.lower_cutoff_hz,
      upper_cutoff_hz: fallback3.upper_cutoff_hz,
      high_slope_100k_1meg_db_dec: fallback3.high_rolloff_100k_1meg_db_dec,
      ac_nrmse: fallback3.ac_nrmse,
      tran_nrmse: fallback3.tran_nrmse,
      power_w: fallback3.power_w,
      area_p: fallback3.area_p,
      notes: fallback3.decision_note ?? "3BJT performance fallback",
    },
    {
      version: selected.stem,
      role: "selected_autotopo",
      score: selected.score,
      midgain_db: selected.midgain_db,
      lower_cutoff_hz: selected.lower_cutoff_hz,
      upper_cutoff_hz: selected.upper_cutoff_hz,
      high_slope_100k_1meg_db_dec: selected.high_slope_100k_1meg,
      ac_nrmse: selected.ac_nrmse,
      tran_nrmse: selected.tran_nrmse,
      power_w: selected.worst_power_w,
      area_p: selected.estimated_area_p,
      notes: selected.notes,
    },
  ].filter(Boolean);
  await fs.writeFile(`${tableDir}/bjt2_autotopo_vs_baselines.csv`, toCsv(comparisonHeaders, baselineRows.map((row) => ({
    version: row.version,
    role: row.role,
    score: row.score === "" ? "" : fmt(Number(row.score), 8),
    midgain_db: row.midgain_db === "" ? "" : fmt(Number(row.midgain_db), 8),
    lower_cutoff_hz: fmt(Number(row.lower_cutoff_hz), 8),
    upper_cutoff_hz: fmt(Number(row.upper_cutoff_hz), 8),
    high_slope_100k_1meg_db_dec: fmt(Number(row.high_slope_100k_1meg_db_dec), 4),
    ac_nrmse: fmt(Number(row.ac_nrmse), 12),
    tran_nrmse: fmt(Number(row.tran_nrmse), 12),
    power_w: fmt(Number(row.power_w), 12),
    area_p: fmt(Number(row.area_p), 6),
    power_delta_vs_selected_w: fmt(Number(row.power_w) - selected.worst_power_w, 12),
    area_delta_vs_selected_p: fmt(Number(row.area_p) - selected.estimated_area_p, 6),
    ac_nrmse_delta_vs_selected: fmt(Number(row.ac_nrmse) - selected.ac_nrmse, 12),
    tran_nrmse_delta_vs_selected: fmt(Number(row.tran_nrmse) - selected.tran_nrmse, 12),
    notes: row.notes,
  }))));

  await appendRunIndex(selected, priorBest);
}
...
for (const row of rows) row.score = candidateScore(row);

const validRows = rows.filter((row) =>
  !row.log_scan &&
  row.midgain_db >= 36.5 &&
  row.midgain_db <= 42.5 &&
  row.upper_cutoff_hz >= 15000 &&
  row.worst_power_w < 2.358623086371e-4 &&
  row.estimated_area_p < 2.651698129268e8 &&
  row.out_pp >= 0.08 &&
  row.out_pp <= 0.32 &&
...
const priorBest = await readPriorAutotopoBest();
const acceptedBaselineScore = 4.01384128;
const scoreToBeat = priorBest ? Number(priorBest.score) : acceptedBaselineScore;
const selectedImprovesReference = selected.score < scoreToBeat;

if (validRows.includes(selected) && selectedImprovesReference) {
  selected.decision = "promote";
  selected.notes = "lowest relaxed PPA score in this LF rolloff pressure run and better than the current autotopo/reference score";
} else if (validRows.includes(selected)) {
  selected.decision = "keep_searching";
  selected.notes = "meets relaxed hard gates but does not improve the current autotopo/reference score";
} else {
  selected.decision = "abandon_family";
  selected.notes = "no candidate in this LF rolloff pressure run met the relaxed hard gates";
}
...
for (const row of rows) {
  ...
  } else if (row.upper_cutoff_hz < 15000) {
    row.decision = "rejected_hard_bandwidth";
    row.notes = "upper cutoff is below the relaxed 15 kHz hard floor";
  } else if (row.midgain_db < 36.5 || row.midgain_db > 42.5) {
    row.decision = "rejected_gain";
    row.notes = "midband gain is outside the relaxed 36.5 dB to 42.5 dB hard gate";
  } else if (row.output_center <= 2.45 || row.output_center >= 2.55 || row.out_pp < 0.08 || row.out_pp > 0.32) {
    row.decision = "rejected_output";
    row.notes = "output common-mode or transient swing indicates practical output failure";
  } else if (row.worst_power_w >= 2.358623086371e-4 || row.estimated_area_p >= 2.651698129268e8) {
    row.decision = "rejected_ppa_vs_3bjt";
    row.notes = "power or area is worse than the 3BJT performance fallback";
  } else {
    row.decision = "rejected_score";
    row.notes = "valid under relaxed hard gates but lower ranked by transparent relaxed PPA score";
  }
}

await writeAutotopoDeliverables(rows, selected, priorBest);

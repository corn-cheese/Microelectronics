...
const candidates = [
  { suffix: "r10k_c150p", r: "10k", rNumeric: 10e3, c: "150p", cNumeric: 150e-12 },
  { suffix: "r10k_c220p", r: "10k", rNumeric: 10e3, c: "220p", cNumeric: 220e-12 },
  { suffix: "r15k_c150p", r: "15k", rNumeric: 15e3, c: "150p", cNumeric: 150e-12 },
  { suffix: "r15k_c220p", r: "15k", rNumeric: 15e3, c: "220p", cNumeric: 220e-12 },
];
...
function stem(candidate) {
  return `bjt3_sweep_hfbuf_${candidate.suffix}`;
}
...
EBUF vbuf 0 vout_drv 0 1
IOPAMP vdd 0 {IOPAMP}
RBUF1 vbuf vfilt1 ${candidate.r}
CBUF1 vfilt1 0 ${candidate.c}
RBUF2 vfilt1 vout_final ${candidate.r}
CBUF2 vout_final 0 ${candidate.c}
...
print v(b3_base) v(b3_emitter) v(b3_collector) v(vout_drv) v(vfilt1) v(vout_final)
...
write ${rawDir}/${version}_ac.raw frequency v(vin) v(b3_collector) v(vout_drv) v(vfilt1) v(vout_final) gain gain_vv gain_db phase_deg`,
...
wrdata ${csvDir}/${version}_tran.csv v(vin) v(b3_collector) v(vout_drv) v(vfilt1) v(vout_final) i(vdd)
...
wrdata ${csvDir}/${version}_noise_10hz_20khz_total.csv onoise_total inoise_total`,
  ...
  return `* SKY130 BJT 3-stage load-buffer/filter fallback ${kind}: ideal OPAMP follower plus two RC poles, R=${candidate.r}, C=${candidate.c}
...
async function summarizeCandidate(candidate) {
  ...
  const pdc = 5 * istatic;
  const cFinal = candidate.cNumeric + 10e-12;
  const pole1 = 1 / (2 * Math.PI * candidate.rNumeric * candidate.cNumeric);
  const pole2 = 1 / (2 * Math.PI * candidate.rNumeric * cFinal);
  return {
    version: `hfbuf_${candidate.suffix}`,
    stem: version,
    candidate,
    changed_family: "high_cutoff_shape",
    changed_values: `ideal OPAMP follower after vout_drv; two output RC poles R=${candidate.r}, C=${candidate.c}; CLOAD_10P=10p retained at vout_final; OPAMP_FT=2Meg`,
...
if (selected) {
  ...
  selected.notes = "log scan pass; selected because the load-buffer/filter fallback preserves the BJT gain path, keeps 10 pF loaded bandwidth above 20 kHz, and makes far high-frequency rolloff meet the 80 dB/dec hard-target proxy";
...
for (const row of rows) {
  ...
  if (row.log_scan) {
    ...
    row.notes = "log scan pass; rejected because the buffer/filter poles move the upper cutoff below the 20 kHz bandwidth target";
  } else if (row.high_slope_100k_1meg > -78) {
    row.decision = "rejected_gain";
    row.notes = "log scan pass; valid bandwidth/transient behavior, but far high-frequency rolloff is still weaker than the 80 dB/dec hard-target proxy";
  } else {
    row.decision = "rejected_power";
    row.notes = "log scan pass; rejected in favor of the accepted fallback candidate with an upper cutoff closer to 20 kHz";
  }
}
...
const tableRows = rows.map((row) => ({
  ...
  pdc_w: fmt(row.pdc_w),
  estimated_area_note: `adds one ahdLib-equivalent OPAMP follower, two output resistors R=${row.candidate.r}, and two internal capacitors C=${row.candidate.c}; external CLOAD_10P remains ppa_included=false`,
  decision: row.decision,
  notes: `${row.notes}; output filter poles=${fmt(row.output_filter_pole1_hz, 2)} Hz and ${fmt(row.output_filter_pole2_hz, 2)} Hz; high slopes 10k-100k=${fmt(row.high_slope_10k_100k, 2)} dB/dec, 20k-200k=${fmt(row.high_slope_20k_200k, 2)} dB/dec, 100k-1Meg=${fmt(row.high_slope_100k_1meg, 2)} dB/dec${Number.isFinite(row.inoise_total) ? `; target-band noise inoise=${fmt(row.inoise_total)} Vrms; ideal buffer model contributes current but no input/output noise` : ""}`,
}));

await fs.writeFile(`${tableDir}/bjt3_sweep_hfbuf_filter_summary.csv`, toCsv(headers, tableRows));
...
if (selected) {
  await fs.writeFile(`${tableDir}/${selected.stem}_summary.csv`, toCsv(["metric", "value", "unit", "notes"], [
    ...
    { metric: "output_filter_pole1_hz", value: fmt(selected.output_filter_pole1_hz, 2), unit: "Hz", notes: "First post-buffer RC pole estimate" },
    { metric: "output_filter_pole2_hz", value: fmt(selected.output_filter_pole2_hz, 2), unit: "Hz", notes: "Second post-buffer RC pole estimate including 10 pF load" },
    ...
    { metric: "istatic", value: fmt(selected.istatic_a), unit: "A", notes: "OP static supply current including 2 MHz OPAMP current model" },
    { metric: "pdc", value: fmt(selected.pdc_w), unit: "W", notes: "VDD times static current" },
    { metric: "inoise_total", value: fmt(selected.inoise_total), unit: "Vrms", notes: "Integrated input-referred noise from 10 Hz to 20 kHz; ideal buffer adds no noise" },
    ...
    { metric: "decision", value: "accepted", unit: "", notes: "Accepted load-buffer/filter fallback; Cycle G deliverables need refresh before replacing the BJT-only final candidate" },
  ]));
}

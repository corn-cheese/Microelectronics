...
## 2026-05-23 - 2BJT autotopo relaxed PPA run 1

- Hypothesis: Rebalance only the existing collector and post-buffer high-frequency poles around the accepted 2BJT so relaxed PPA improves without adding any new large capacitors or changing the 2BJT signal path.
- Change: Added `maxrun/run_bjt2_autotopo_ppa_sweep.mjs`, extended `maxrun/test_bjt2_runner_structure.mjs`, and generated `bjt2_autotopo_*` netlists/results plus the required autotopo summary, baseline comparison, and run-index tables.
- Candidates: 48 variants in family `existing_pole_rebalance`, sweeping `CH={18p,20p,22p,24p}`, `RBUF={8.2k,10k,12k}`, and `CBUF={180p,200p,220p,240p}` at fixed accepted 2BJT bias/coupling and `OPAMP_FT=250k`.
- Best: `bjt2_autotopo_prb_rc120k_re3p9k_rb4p7m_c68n_cout10n_ch24p_r12k_c180p_ft250k`, score `4.00052558`, `midgain=39.760085 dB`, `fL=12.569960 Hz`, `fH=20254.842141 Hz`, `10 Hz relative=-4.391526 dB`, `20 kHz relative=-2.936274 dB`, `100k-1Meg=-81.0449 dB/dec`, `AC nRMSE=0.133282827101`, `tran nRMSE=0.024183036261`, `power=65.172433 uW`, `area=1.819900e8 p`.
- Compared against:
  - accepted 2BJT: area improves by `9.236946e4 p`, power is effectively unchanged (`+2.434e-14 W`), AC nRMSE improves by `0.0030366074`, transient nRMSE worsens by `0.000449358452`, upper cutoff drops by about `154.538 Hz`, and far HF slope gains about `0.4855 dB/dec` of margin.
  - prior autotopo best: none available before this run; `bjt2_autotopo_run_index.csv` had only the header.
  - 3BJT fallback: keeps the practical PPA advantage with about `170.690 uW` lower power and `8.317978e7 p` lower area, but remains weaker on AC nRMSE (`+0.027382435057`) and far-rolloff margin.
- Decision: promote as the current best relaxed-PPA 2BJT autotopo candidate, with the caveat that the low-frequency shape is not improved.
- Next: Try interstage low-frequency shaping only if the next run can improve the 10 Hz shape without giving back the area/power advantage.
...
## 2026-05-23 - 2BJT autotopo relaxed PPA run 2

- Hypothesis: Split the interstage coupling capacitor between the first collector and second base into two smaller high-impedance high-pass sections, adding a midpoint rebias divider, to test whether capacitor-area reduction and improved AC nRMSE can outweigh low-frequency shape penalties.
- Change: Updated `maxrun/run_bjt2_autotopo_ppa_sweep.mjs` from run 1 pole rebalance to family `interstage_low_frequency_shaping`; added split `C12A/vlf_mid/C12B` netlist generation, `RLF_TOP/RLF_BOT` midpoint rebias, split-interstage area accounting, and run-2 scoring/decision text. Updated `maxrun/test_bjt2_runner_structure.mjs` to require the run-2 family and split interstage netlist.
- Candidates: 48 variants in family `interstage_low_frequency_shaping`, sweeping `C12A={10n,15n,22n,33n}`, `C12B={10n,15n,22n,33n}`, and `RLF={10Meg,15Meg,22Meg}` with the prior best HF settings `CH=24p`, `RBUF=12k`, `CBUF=180p`, `OPAMP_FT=250k`.
- Best: `bjt2_autotopo_ilf_rc120k_re3p9k_rb4p7m_cin68n_c12a33n_c12b33n_rlf22m_cout10n_ch24p_r12k_c180p_ft250k`, score `6.41780256`, `midgain=39.692090 dB`, `fL=28.574866 Hz`, `fH=20281.084004 Hz`, `10 Hz relative=-11.742238 dB`, `20 kHz relative=-2.915019 dB`, `100k-1Meg=-81.0193 dB/dec`, `AC nRMSE=0.122690143332`, `tran nRMSE=0.020146517849`, `power=65.737489 uW`, `area=1.840133e8 p`.
- Compared against:
  - accepted 2BJT: AC nRMSE improves by `0.013629291169` and transient nRMSE improves by `0.003587159960`, but power worsens by `0.565056 uW`, area worsens by `1.930934e6 p`, lower cutoff rises by about `16.005 Hz`, and 10 Hz relative gain worsens to `-11.742238 dB`.
  - prior autotopo best: score worsens by `2.41727698`, AC nRMSE improves by `0.010592683769`, transient nRMSE improves by `0.004036518412`, power worsens by `0.565056 uW`, area worsens by `2.023334e6 p`, and low-frequency shape is materially worse.
  - 3BJT fallback: keeps practical PPA advantage with about `170.125 uW` lower power and `8.115648e7 p` lower area, but remains worse on AC nRMSE by `0.016789751288` and far-rolloff margin.
- Decision: keep searching; the family is valid under relaxed hard gates but the low-frequency penalty dominates the lower AC/transient nRMSE.
- Next: Rotate to an area-aware output split only if it can reduce area or power, otherwise try limited emitter degeneration bypass shaping.

## 2026-05-23 - 2BJT autotopo relaxed PPA run 3

- Hypothesis: Split the output coupling capacitor into two smaller high-impedance high-pass sections, adding one midpoint rebias divider, to test whether lower capacitor area can improve relaxed PPA without losing the 2BJT power advantage.
- Change: Updated `maxrun/run_bjt2_autotopo_ppa_sweep.mjs` from run 2 interstage shaping to family `area_aware_output_split`; restored the single 68 nF interstage C12 path, added `COUT1/vhp_mid/COUT2` output split generation, `RHP_TOP/RHP_BOT` midpoint rebias, split-output area accounting, and run-3 scoring/decision text. Updated `maxrun/test_bjt2_runner_structure.mjs` to require the run-3 family and output split netlist.
- Candidates: 48 variants in family `area_aware_output_split`, sweeping `COUT1={2.2n,3.3n,4.7n,6.8n}`, `COUT2={2.2n,3.3n,4.7n,6.8n}`, and `RHP={10Meg,15Meg,22Meg}` with fixed `ROUT=10Meg`, `CH=24p`, `RBUF=12k`, `CBUF=180p`, and `OPAMP_FT=250k`.
- Best: `bjt2_autotopo_aos_rc120k_re3p9k_rb4p7m_c68n_cout16p8n_cout26p8n_rhp22m_rout10m_ch24p_r12k_c180p_ft250k`, score `4.97462927`, `midgain=39.667942 dB`, `fL=16.879805 Hz`, `fH=20298.190360 Hz`, `10 Hz relative=-7.130373 dB`, `20 kHz relative=-2.925786 dB`, `100k-1Meg=-81.0226 dB/dec`, `AC nRMSE=0.123158520923`, `tran nRMSE=0.023615229755`, `power=65.740685 uW`, `area=1.908513e8 p`.
- Compared against:
  - accepted 2BJT: AC nRMSE improves by `0.013160913578` and transient nRMSE improves by `0.000118448054`, but power worsens by `0.568252 uW`, area worsens by `8.768941e6 p`, lower cutoff rises by about `4.310 Hz`, and 10 Hz relative gain worsens to `-7.130373 dB`.
  - prior autotopo best: score worsens by `0.97410369`, AC nRMSE improves by `0.010124306178`, transient nRMSE improves by `0.000567806506`, power worsens by `0.568252 uW`, area worsens by `8.861341e6 p`, and low-frequency shape is materially worse.
  - 3BJT fallback: keeps practical PPA advantage with about `170.122 uW` lower power and `7.431847e7 p` lower area, but remains worse on AC nRMSE by `0.017258128879` and far-rolloff margin.
- Decision: keep searching; the family is valid under relaxed hard gates but does not reduce area after the added rebias divider and worsens low-frequency shape enough to lose to run 1.
- Next: Try limited emitter degeneration bypass shaping, keeping capacitor values small and using the run-1 HF settings as the reference.
